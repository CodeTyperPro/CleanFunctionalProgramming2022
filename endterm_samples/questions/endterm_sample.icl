...
instance == Human
where (==) a b = (a.age == b.age) && (abs (a.height-b.height) <=2) 
//Start = Leo == Rose // False
//Start = Harry == Harry // True 
//Start = Harry == Harry2 //True
...
minMaxDiff ar n = {t\\ t<-: ar | prop t n}
where
	prop :: [Int] Int -> Bool
	prop ls n = (a - b) > n
	where
...
tb :: [Int] -> Real
tb ls = (toReal (sum ls )) / (toReal (length ls))

av :: Player -> Real
av {player_name,goals,moral = Happy} = (tb goals) + (toReal(length goals))*0.1
av {player_name,goals,moral = Sad}  = (tb goals)

instance < Player
where (<) a b = (av a) < (av b)

bestStriker :: [Player] -> String 
bestStriker ls = choose.player_name
where choose = last (sort ls)
...
findWinner :: Fighter Fighter Int -> (String, Int)
findWinner a b n
| n == 10 && a.health_points > 0 && b.health_points > 0 = ("No winner", 22)
| a.health_points <= 0 = (b.fighter_name,n)
| b.health_points <= 0 = (a.fighter_name,n)
| n rem 2 ==0 && a.attacking_points >= b.defending_points = findWinner a {b & health_points = b.health_points - (a.attacking_points - b.defending_points)} (n+1)
| n rem 2 ==0  && a.attacking_points < b.defending_points =  findWinner a b (n+1)
| n rem 2 ==1 && b.attacking_points >= a.defending_points = findWinner {a & health_points = a.health_points - (b.attacking_points - a.defending_points)} b (n+1)
| n rem 2 ==1 && b.attacking_points < a.defending_points = findWinner a b (n+1)

getWinnerRound :: Fighter Fighter -> (String, Int)
getWinnerRound a b = findWinner a b 0
