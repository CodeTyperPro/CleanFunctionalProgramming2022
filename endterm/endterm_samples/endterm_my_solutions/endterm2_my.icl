...
shiftCipher str x = {shift (toUpper i) x \\ i<-: str}
...
* if we can get signals, i.e. 

all are within the radius,
...
isInside :: Circle Point -> Bool
isInside x y = (distance x.center y) < x.radius 

check :: Circle {Point} -> Bool
check c array = all ((==) True) [ (isInside c i) \\ i <-: array]
...
instance == Book
where 
	(==) infixl 6 :: !Book !Book -> Bool
	(==) x y = (x.title == y.title) && (x.author == y.author) && (x.pyear == y.pyear) && (x.num_of_pages == y.num_of_pages)
...
instance + Library
where 
	(+) infixl 6 :: !Library !Library -> Library
	(+) x y = {lib_name = concat, books = joinedBooks}
	where
		concat = x.lib_name +++ y.lib_name
